"""
训练器（独立实现）- 支持 2D / 3D / VLA / VLA-Simple / RGB-Action
"""
import os
...
class Trainer:
    ...
    def _load_data(self):
        ...
        if model_type in ['rgb_action', 'rgb_vla']:
            ...
            self.dataset = self.strategy.load(
                ...
                max_length=64,
                num_steps=dataset_cfg.get('num_steps', 1)
            )
            print(f"[Train] Loaded {len(self.dataset)} samples")
        elif model_type in ['voxel', 'vla', 'vla_simple']:
            ...
            self.dataset = self.strategy.load(
                ...
                transform=self.strategy.get_transform(voxel_spec),
    ...
    def _build_model(self):
        ...
        if model_type == 'voxel':
            ...
            encoder = VoxelJEPAEncoder(
                spec=voxel_spec,
                latent_dim=embed_dim,
                token_dim=model_cfg.get("jepa_encoder", {}).get("token_dim", 128),
            )
            ...
            self.model = VLAJEPA(
                ...
                projector=projector,
                pred_proj=pred_proj,
                llm_model_name=model_cfg.get("llm_model_name", "gpt2"),
                llm_dim=model_cfg.get("llm_dim", 768),
                jepa_dim=embed_dim,
                jepa_token_dim=model_cfg.get("jepa_token_dim", 128),
                num_heads=model_cfg.get("num_heads", 8),
                use_adaln=model_cfg.get("use_adaln", True),
                freeze_llm=model_cfg.get("freeze_llm", True),
                num_actions=model_cfg.get("num_actions", 8),
            )
        ...
        elif model_type in ['rgb_action', 'rgb_vla']:
            from transformers import AutoTokenizer
            tokenizer = AutoTokenizer.from_pretrained('prajjwal1/bert-tiny')
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
        ...
        # SIGReg 只在有 loss.sigreg 配置时使用
        if hasattr(self.cfg, 'loss') and hasattr(self.cfg.loss, 'sigreg'):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch in pbar:
            ...
            if self.sigreg is not None and "emb" in output:
